...
export default class NFCScanner {
  ...
  setScanAnimation(state) {
    this.scanAnimation = state
    this.notifyAnimationCallbacks("scanAnimation", state)
  }

  setPulseAnimation(state) {
    this.pulseAnimation = state
    this.notifyAnimationCallbacks("pulseAnimation", state)
  }

  setSuccessAnimation(state) {
    this.successAnimation = state
    this.notifyAnimationCallbacks("successAnimation", state)
  }

  showStudentDetailsModal(userData, eventData) {
    this.studentModalData = userData
    this.eventModalData = eventData
    this.showStudentModal = true
    this.notifyAnimationCallbacks("showStudentModal", true)
  }
  ...
  hideStudentDetailsModal() {
    ...
    this.studentModalData = null
    this.eventModalData = null
    this.notifyAnimationCallbacks("showStudentModal", false)
  }
  ...
  notifyAnimationCallbacks(type, state) {
  ...
  removeElectronEventListeners() {
    ...
    if (this.electronSerialErrorHandler) {
      window.removeEventListener("serial-error", this.electronSerialErrorHandler)
    }
  }
  ...
  isSiliconLabsDevice(port) {
    try {
      ...
      if (this.isElectron && port.vendorId && port.productId) {
        return port.vendorId.toLowerCase() === "10c4" && port.productId.toLowerCase() === "ea60"
      }
      return false
    }
  }
  ...
  getFriendlyPortName(port, index) {
    try {
      ...
      if (this.isElectron && port.vendorId && port.productId) {
        ...
        const name = NFCScanner.USB_NAME_MAP[id] || `Serial Device (${port.path})`
        return name
      }
  ...
  async connectToDeviceBrowser() {
    ...
    if (!targetPort) {
      try {
        console.log("No Silicon Labs device found in available ports, requesting user selection")
        ...
        targetPort = port
        console.log("User selected port:", this.getFriendlyPortName(targetPort, 0))
      } catch (err) {
    ...
    if (targetPort) {
      ...
      throw new Error("No NFC Scanner device found. Please connect your device and try again.")
    }
  }
    ...
    try {
      ...
      if (this.port) {
        ...
        if (this.isElectron) {
          ...
          try {
            if (window.SerialBridge && typeof window.SerialBridge.closePort === "function") {
  ...
  updateFileRecord = async (eventId, fileName, filePath, downloadURL, fileSize, isPublic, storageFolder) => {
  ...
  updateAttendanceSheet = async (userData, eventData, eventId, registeredDate, registeredTimestamp) => {
    try {
      ...
      const storageFolder = eventData.isPublic ? "public/events" : `${this.currentUserData.uid}/event_data`
  ...
  async registerUserForEvent(userData, eventId, currentUser) {
    try {
      ...
      const { exists: sheetExists, documentData } = await this.checkEventAttendanceSheet(eventId)

      // Now update the attendance sheet
      await this.updateAttendanceSheet(userData, eventData, eventId, registeredDate, now.toISOString())

      // Show student details modal
      this.showStudentDetailsModal(userData, eventData)

      // Trigger success animation
      this.setSuccessAnimation(true)
      setTimeout(() => {
        this.setSuccessAnimation(false)
      }, 3000)
      ...
      if (error.message.includes("Unsupported field value: undefined (found in field course")) {
        throw new Error("Access restricted to students only")
      }
      throw error
    }
  }
  ...
  async readNfcTag() {
    if (!this.port) {
      throw new Error("No NFC Scanner connected. Please connect your device first.")
    }
    ...
    this.setScanAnimation(true) // Start scan animation
    ...
    try {
      ...
      this.isScanning = false
      this.setScanAnimation(false) // Stop scan animation
    }
  }
  ...
  async readNfcTagElectron() {
    return new Promise((resolve, reject) => {
      ...
      const isResponseComplete = (text) => {
        return text.includes("Read successful:") || text.includes("No NFC detected") || text.includes("Read failed!")
      }
        ...
        if (isResponseComplete(receivedText) && !responseComplete) {
          ...
          setTimeout(() => {
            ...
            if (receivedText.includes("Read successful:")) {
              ...
              reject(new Error("No NFC tag detected. Please bring an NFC tag closer to the reader."))
            } else if (receivedText.includes("Read failed!")) {
              reject(new Error("NFC scan failed! Please wait for 5 seconds before removing the card and try again."))
            } else {
      ...
      while (!responseComplete) {
        ...
        if (isResponseComplete(receivedText)) {
          ...
          if (receivedText.includes("Read successful:")) {
            ...
            throw new Error("No NFC tag detected. Please bring an NFC tag closer to the reader.")
          } else if (receivedText.includes("Read failed!")) {
            throw new Error("NFC scan failed! Please wait for 5 seconds before removing the card and try again.")
          }
        }
      }
  ...
  async writeNfcTagElectron(text) {
    return new Promise((resolve, reject) => {
      ...
      // Function to determine if response is complete
      const isResponseComplete = (text) => {
        return text.includes("Write successful") || text.includes("No NFC detected") || text.includes("Write failed!")
      }
        ...
        if (isResponseComplete(receivedText) && !responseComplete) {
          ...
          setTimeout(() => {
            ...
            if (receivedText.includes("Write successful")) {
                ...
                this.verifyWrittenData(text)
                  .then((verifyResult) => {
                    if (verifyResult.verified) {
                      ...
                      console.warn("Write reported success but verification failed")
                      resolve({
                        success: true,
                        writtenData: text,
                        fullResponse: receivedText,
                  ...
                  .catch((err) => {
                    ...
                    resolve({
                      ...
                      fullResponse: receivedText,
                      verified: false,
                      message: "Write reported as successful but verification could not be completed.",
                    })
                  })
              }, 2000) // Wait 2 seconds before verification for NDEF format
            } else if (receivedText.includes("No NFC detected")) {
              reject(new Error("No NFC tag detected. Please hold the NFC tag directly against the scanner."))
              ...
              reject(new Error("Unknown write result. Please check the NFC tag and try again."))
            }
  ...
  async writeNfcTagBrowser(text) {
    ...
    try {
      ...
      while (!responseComplete) {
        ...
        // Check if we timed out
        if (result.timeout) {
          throw new Error("Write operation timed out. Please try again and make sure the NFC tag stays in range.")
        }
        ...
        if (isResponseComplete(receivedText)) {
          ...
          if (receivedText.includes("Write successful")) {
            ...
            try {
              ...
              if (verifyResult.verified) {
                ...
                return {
                  ...
                  fullResponse: receivedText,
                  verified: false,
                  message: "Write reported as successful but verification failed. Try again or check the NFC tag.",
                }
              }
            } catch (verifyError) {
            ...
            throw new Error("No NFC tag detected. Please hold the NFC tag directly against the scanner.")
          } else if (receivedText.includes("Write failed!")) {
            throw new Error(
              "NFC write failed! The tag might be read-only or damaged. Please try again with a different tag.",
            )
      ...
      throw new Error("Write operation did not complete successfully. Please try again.")
    } finally {
      // Clear timeout if it's still active
      if (timeoutId) {
        clearTimeout(timeoutId)
  ...
  async verifyWrittenData(expectedText) {
    try {
      ...
      if (readResult.success && readResult.tagData) {
        ...
        console.log(`Verification - Expected: "${expectedTrimmed}", Read: "${tagText}"`)
    ...
    while (retries <= maxRetries) {
      try {
        ...
        if (retries > maxRetries) {
          throw new Error(`Failed to send command after ${maxRetries} attempts: ${error.message}`)
        }
